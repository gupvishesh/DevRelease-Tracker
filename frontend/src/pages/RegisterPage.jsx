    ...
    <div className="flex" style={{ height: '100vh', backgroundColor: 'var(--color-paper)' }}>
      ...
      <div className="flex items-center justify-center" style={{ flex: 1 }}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6" style={{ width: '400px', marginLeft: '10%' }}>
          <h2 className="editorial" style={{ fontSize: '24px' }}>Register Clearance</h2>
          {error && <div className="text-accent editorial">{error}</div>}
          <div className="flex flex-col gap-2">
            <label className="label-caps">Full Name</label>
            <input type="text" value={name} onChange={e=>setName(e.target.value)} required style={{ border: 'none', borderBottom: 'var(--border)', padding: 'var(--space-2) 0', backgroundColor: 'transparent', outline: 'none' }} />
          </div>
          <div className="flex flex-col gap-2">
          ...
          <Button type="submit" style={{ width: '100%', marginTop: 'var(--space-4)' }}>Request Access</Button>
          <div className="editorial text-muted" style={{ textAlign: 'center' }}>
            Already cleared? <Link to="/login" className="text-ink" style={{ textDecoration: 'underline' }}>Login</Link>
          </div>
        </form>
      </div>
    </div>
